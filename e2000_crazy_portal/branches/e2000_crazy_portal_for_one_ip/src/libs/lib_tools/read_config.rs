...
pub fn read_user_config() -> Result<ConfigData> {
    ...
    let (start_ip, static_mask, static_gw) = if use_ipv6 {
        (
            ...
            IpAddr::V6(Ipv6Addr::new(0xffff, 0xffff, 0xffff, 0xffff, 0, 0, 0, 0)), // Prefix length is converted later
    ...
    Ok(ConfigData {
        ...
        static_mask,
        static_gw,
        portal_ip: value.get("PORTAL_SERVER_IP").and_then(|v| v.get("portal_ip_or_ipv6")).and_then(|v| v.as_str()).unwrap_or("0.0.0.0").parse().unwrap_or(IpAddr::V4(Ipv4Addr::new(0,0,0,0))),
        portal_user_num: value.get("PORTAL_USER_NUM").and_then(|v| v.get("portal_user_num")).and_then(|v| v.as_integer()).unwrap_or(0) as usize,
        portal_user_head: value.get("PORTAL_USER_NAME").and_then(|v| v.get("portal_user_head")).and_then(|v| v.as_str()).unwrap_or("").to_string(),
        portal_user_tail: value.get("PORTAL_USER_NAME").and_then(|v| v.get("portal_user_tail")).and_then(|v| v.as_integer()).unwrap_or(0) as usize,
        all_password: value.get("PORTAL_USER_PASSWD").and_then(|v| v.get("all_password")).and_then(|v| v.as_str()).unwrap_or("").to_string(),
        port: value.get("PORTAL_PORT").and_then(|v| v.get("portal_port")).and_then(|v| v.as_integer()).unwrap_or(80) as u16,
