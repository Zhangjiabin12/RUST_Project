...
pub async fn run_portal_reauth(tcpsocket_mutex1: &Arc<Mutex<TcpSocket>>,tcpsocket_mutex2: &Arc<Mutex<TcpSocket>>, index: usize, user_config: &Arc<Mutex<UserConfig>>, ) -> anyhow::Result<()> {
    
    let moved_socket_clone = tcpsocket_mutex1.clone();
    let portal_socket_clone = tcpsocket_mutex2.clone();

    let user = user_config.lock().await;
    let  moved_socket = moved_socket_clone.lock().await;
    let  portal_socket = portal_socket_clone.lock().await;

    let mut moved_stream = moved_socket.connect(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(1,2,3,4)), 80)).await.unwrap();
    let mut portal_stream = portal_socket.connect(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(100,100,1,32)),80)).await.unwrap();
...
// loop {
    ...
    moved_stream.write_all(&http_request);
...
// 构建新请求的URI
    ...
    info!("STA{} Send offline to portal server",index);
    sleep(Duration::from_secs(10)).await;
    info!("STA{} Restart Portal Auth", index);
    let stream = TcpStream::connect("1.2.3.4:80");
