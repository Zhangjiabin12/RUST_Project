...
pub fn read_user_config() -> Result<ConfigData> {
    ...
    Ok(ConfigData {
        ...
        user_same_user: value.get("PORTAL_USER").and_then(|v| v.get("use_same_user")).and_then(|v| v.as_bool()).unwrap_or(false),
        offline: value.get("PORTAL_USER").and_then(|v| v.get("offline")).and_then(|v| v.as_bool()).unwrap_or(false),
        offline_time: value.get("PORTAL_USER").and_then(|v| v.get("offline_time")).and_then(|v| v.as_integer()).unwrap_or(0) as u64,
