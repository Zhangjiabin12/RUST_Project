...
const STA_BASE_MAC_PREFIX: &str = "00017a";
...
impl InterfaceManager {
    pub fn new(user_config: &ConfigData) -> Result<i8, Error> {
        let mut iface_config = InterfaceManager {
            ...
            base_mac: String::from(STA_BASE_MAC_PREFIX),
    ...
    fn create_interface(&mut self) -> Result<(),Error> {
        ...
        for (i, ip) in ip_addr_list.iter().enumerate() {
            ...
            self.run_command(&format!("ip link add link {} name {} type macvlan mode bridge", self.parent_interface, name)).unwrap();
            ...
            self.run_command(&format!("route add -net {}/{} gw {} dev {}", network_addr, cidr, self.gw, name, )).unwrap();
            pb.inc(1);
        }
        pb.finish();
