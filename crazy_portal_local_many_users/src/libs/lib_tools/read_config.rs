    ...
    for line in contents.lines() {
        ...
        if let Some((key, value)) = line.split_once('=') {
            match key.trim() {
                ...
                "portal_reauth" => config_data.portal_reauth = value.trim().parse().map_err(|_| format!("portal_con have Invalid value for portal_reauth: {}", value.trim())).unwrap_or_default(),
                "sta_mac_header" => config_data.sta_mac_header = value.trim().parse().map_err(|_| format!("portal_con have Invalid value for sta_mac_header: {}", value.trim())).unwrap_or_default(),
