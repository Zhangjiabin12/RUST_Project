...
pub async fn run_portal(tcpsocket_mutex1: &Arc<Mutex<TcpStream>>,tcpsocket_mutex2: &Arc<Mutex<TcpStream>>, index: usize, user_config: &Arc<Mutex<UserConfig>>, ) -> anyhow::Result<()> {
    ...
    loop {
        ...
        match result {
            ...
            Ok(_n) => {
                info!("STA{} Read{} bytes", index, _n);
                debug!("STA{} Read{} bytes : {:?}",index, _n, &data_buffer);
                break;
            }
            Err(e) => {
    ...
    loop {
        ...
        match result {
            Ok(0) => {
                ...
                portal_stream.write_all(&request_bytes).await.unwrap();
                if loop_flags >= 3 {
                    info!("STA{} connection closed or cant break", index);
                    break;
                }
                break;
            }
            Ok(_n) => {
                info!("STA{} Get Some Response Bytes len {}", index, _n);
                break;
            }
            Err(e) => {
                error!("STA{} read error: {}", index, e);
