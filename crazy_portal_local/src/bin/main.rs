...
async fn main() -> anyhow::Result<()> {
    ...
    match portal_reauth {
        ...
        1 => {
            ...
            //     let bind_socket_moved = Arc::new(Mutex::new(tcp_socket_bind_device_socket(&Arc::new(format!("{}.{}",interface_name,user_index+1)), *ip, portal_config.port,portal_config.redirect_ip).await.unwrap()));
            //     let bind_socket_portal = Arc::new(Mutex::new(tcp_socket_bind_device_socket(&Arc::new(format!("{}.{}",interface_name,user_index+1)), *ip, portal_config.port,portal_config.portal_ip).await.unwrap()));
